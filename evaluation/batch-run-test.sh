...
run_ns3() {
    ...
    tot_waste=$(python3 /home/xuzy/UplinkCoordination/evaluation/log-process.py -l "${ns3_output}" -w)
    ...
    # clean output
    echo $policy, $nclient, $seed, $avg_thp, $min_thp, $tail_thp, $tot_waste >> $output_file_clean
}
...
echo "policy, nclient, seed, avg_thp, min_thp, tail_thp, tot_waste" > ${RESULT_DIR}/result_p2p.csv

parallel -j${CORE_COUNT} run_ns3 ::: ${policies[@]} ::: ${seeds[@]} ::: ${nclients[@]} ::: ${simTime}

echo "policy, nclient, seed, avg_thp, min_thp, tail_thp, tot_waste" > ${RESULT_DIR}/result_p2p_tack.csv

parallel -j${CORE_COUNT} run_ns3_tack ::: ${policies0} ::: ${seeds[@]} ::: ${nclients[@]} ::: ${simTime} ::: ${ackmaxcounts[@]}
