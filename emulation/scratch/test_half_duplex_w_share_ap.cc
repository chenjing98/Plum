...
std::map<ns3::Ipv4Address,uint32_t> ip_to_node[80];
uint32_t m_lastN_id;
...
void BandwidthTrace(TraceElem elem, uint32_t n_client)
{
    ...
    total_bw = total_bw * elem.channel_prop;
...
int main(int argc, char *argv[])
{
    ...
    // if(seed == 14) id_seed = 1; 
    // if(seed == 20) id_seed = 2; 
    // if(seed == 25) id_seed = 3; 
    // if(seed == 27) id_seed = 4; 
    // if(seed == 30) id_seed = 5; 
    ...
    if(nClient == 12) id_ncli = 5;
    if(nClient == 14) id_ncli = 6;
    if(nClient == 16) id_ncli = 7;
    if(nClient == 18) id_ncli = 8;
    if(nClient == 20) id_ncli = 9;
    m_lastN_id = (id_seed-1)*10 + id_ncli;
    // m_lastN_id = id_seed;
    ...
    std::string trace_file_clinet_0;
    ...
    for (uint32_t i = 0; i < nClient; i++)
    {
        ...
        if (vary_bw)
        {
            ...
            if (i == 0)
            {
                trace_file_clinet_0 = tracefile;
                elem = {tracefile,
                        static_cast<TRACE_MODE>(trace_mode),
                        static_cast<DATASET>(dataset),
                        ulDevices[i].Get(0),
                        dlDevices[i].Get(1),
                        trace_interval,
                        simulationDuration,
                        (double_t)maxBitrateKbps / 1000.,
                        minBitrateKbps / 1000.,
                        server_bottleneck_mbps / (double_t)nClient,
                        i, 0.5, ul_prop};
            }
            else if (i == 1)
            {
                elem = {trace_file_clinet_0,
                        static_cast<TRACE_MODE>(trace_mode),
                        static_cast<DATASET>(dataset),
                        ulDevices[i].Get(0),
                        dlDevices[i].Get(1),
